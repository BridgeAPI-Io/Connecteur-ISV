...
class BridgeGateway extends \WC_Payment_Gateway
{
	...
	public function __construct()
	{
		...
		$this->method_title = __('Bank Transfer by Bridge', 'bridgeapi-io');
		$this->method_description = __('Bridge Instant Transfer Payment Plug-in for WooCommerce', 'bridgeapi-io');
		$this->title = __('Instant bank payment', 'bridgeapi-io');
		$this->description = __('Pay with a click. No IBAN registration needed, secured by your bank, no additional fee.', 'bridgeapi-io');
		$this->description .= sprintf('<p>%s <img src="%s/images/banque_de_france.png" /></p>', __('Payment solution authorized by', 'bridgeapi-io'), BRIDGEAPI_IO_ASSETS_URI);
		$this->description .= sprintf('<p id="bridge-io-toggleCollapse" onclick="collapse()">%s <img src="%s/images/arrow-down-sign-to-navigate.png" /></p>', __('How it works?', 'bridgeapi-io'), BRIDGEAPI_IO_ASSETS_URI);
